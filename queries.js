...
function getAllCoupons(req, res, next) {
  let sql = 'select id, number, used, discount_percentage from coupon where deleted = false';
...
function createCoupon(req, res, next) {
  const sql = 'insert into coupon (number, discount_percentage)' + 'values($1, $2)';
  const value = [req.body.number, req.body.discount_percentage];
...
function updateCoupon(req, res, next) {
  const sql = 'update coupon set number=$1, used=$2, discount_percentage=$3 where id=$4';
  const value = [req.body.number, req.body.used, req.body.discount_percentage, parseInt(req.params.id)];
  writeData(sql, value, (data) => {
